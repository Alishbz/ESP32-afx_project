 ...
 * on an ESP32 microcontroller. It utilizes the microcontroller's GPIO and LEDC functionalities
 * to provide precise control over the color and brightness of each LED.
 * The library supports the configuration of each LED's color, as well as enabling
 * various lighting effects including blinking, transition, and breathing effects,
 * with customizable timing parameters. Each RGB LED can be individually controlled,
 * allowing for complex light effects across multiple LEDs.
 *
 * @note This implementation is compatible with the ESP-IDF development framework, version 5.0.2.
 *
 * @note This software is licensed under the Creative Commons Attribution-NonCommercial-NoDerivatives 4.0
 * International License. To view a copy of this license, visit http://creativecommons.org/licenses/by-nc-nd/4.0/.
 * You may not use, distribute, modify, or sell this software without the express permission of the author.
 * For more information, please contact the author at massimodallabona@gmail.com.
 */
...
#define MIN(a, b) (((a) < (b)) ? (a) : (b)) // Macro function to get the minimum value between two numbers.
 ...
 *
 * @description This function sets up one of the ESP32's LEDC (LED Controller) channels to control a specific LED.
 * It first sets up a LEDC timer with a predefined configuration (8-bit duty resolution, 5000 Hz frequency, high-speed mode, timer number 0).
 * It then configures the LEDC channel to use the provided GPIO and the LEDC timer. The LED is initially off (duty = 0).
 * The function returns ESP_OK if the setup was successful. If there was an error, it returns an appropriate ESP_ERR code.
 *
 ...
 *
 * @description This function sets the color of an RGB LED by setting the duty cycle of the LEDC channels
 * controlling the red, green, and blue LEDs. The color is specified as a 24-bit RGB color value.
 * The red, green, and blue components are extracted from the color value, and the LEDC duty cycle
 * is set proportional to the component value. Since the LEDC timer resolution is 8 bits, the color
 * component values are used directly as the duty cycle. If an error occurs while setting the duty
 * cycle, an appropriate ESP_ERR code is returned.
 *
 * @param rgb_led A pointer to the rgb_led_t structure that contains the configuration of the RGB LED.
 * @param color The 24-bit RGB color value. The red component is in the most significant 8 bits, followed by green, and then blue.
 * @return esp_err_t ESP_OK if successful, or an error code if not.
 */
 ...
 *
 * @description This function initializes the ESP32's LEDC (LED Controller) hardware peripheral to control an RGB LED.
 * The RGB LED is represented by an `rgb_led_cfg_t` struct which contains the GPIO numbers and LEDC channels for the red, green, and blue LEDs.
 * The function first checks if the provided GPIO numbers are valid for output, and if the LEDC channels are not already in use.
 * It then configures the GPIOs for output and sets up the LEDC peripheral to control the LEDs.
 * The function returns ESP_OK if the initialization was successful. If there was an error, it returns an appropriate ESP_ERR code.
 *
 * @param config Pointer to an `rgb_led_cfg_t` struct containing the configuration data for the RGB LED.
 * @return esp_err_t ESP_OK if successful, or an error code if not.
 */
esp_err_t rgb_led_init(rgb_led_t *rgb_led_cfg)
{
    ...
    ESP_GOTO_ON_FALSE(GPIO_IS_VALID_OUTPUT_GPIO(led.red_pin), ESP_ERR_INVALID_ARG, err, TAG, "Invalid red_pin");
    ESP_GOTO_ON_FALSE(GPIO_IS_VALID_OUTPUT_GPIO(led.green_pin), ESP_ERR_INVALID_ARG, err, TAG, "Invalid green_pin");
    ...
    ESP_RETURN_ON_ERROR(setup_ledc(led.red_pin, led.red_channel), TAG, "Failed to setup RED LED");

    // Initialize LEDC for the green LED
    ESP_RETURN_ON_ERROR(setup_ledc(led.green_pin, led.green_channel), TAG, "Failed to setup GREEN LED");

    // Initialize LEDC for the blue LED
    ESP_RETURN_ON_ERROR(setup_ledc(led.blue_pin, led.blue_channel), TAG, "Failed to setup BLUE LED");
    ...
    if (num_led_effects >= MAX_RGB_LEDS) {
        ...
        return ESP_FAIL; // Maximum number of LEDs reached, unable to add LED effect
    }
 ...
 */
rgb_led_t rgb_led_new(gpio_num_t red_pin, gpio_num_t green_pin, gpio_num_t blue_pin, ledc_channel_t red_ch, ledc_channel_t green_ch, ledc_channel_t blue_ch)
{
    rgb_led_t led;
    led.red_pin = red_pin;
 ...
 *
 * This function stops the active transition effect for the specified LED. It stops and deletes the associated timer,
 * resets the transition effect data, and updates the effect state to indicate that no effect is active.
 *
...
esp_err_t rgb_led_stop_transition_effect(rgb_led_t *led)
{
    ...
    if (led_effects[i].effect_state == RGB_LED_EFFECT_TRANSITION) {
        ...
        if (err == ESP_ERR_INVALID_ARG) {
            ESP_LOGE(TAG, "Failed to stop transition effect timer: %s", esp_err_to_name(err));
            return err;
        }

        err = esp_timer_delete(led_effects[i].transition_effect.timer);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Failed to delete transition effect timer: %s", esp_err_to_name(err));
            return err;
        }
 ...
 *
 * This function stops the active blink effect for the specified LED. It stops and deletes the associated timer,
 * resets the blink effect data, and updates the effect state to indicate that no effect is active.
 *
 * @param led Pointer to the LED structure.
 * @return
...
esp_err_t rgb_led_stop_blink_effect(rgb_led_t *led)
{
    ...
    if (led_effects[i].effect_state == RGB_LED_EFFECT_BLINK) {
        ...
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Failed to stop blink effect timer: %s", esp_err_to_name(err));
            return err;
        }

        err = esp_timer_delete(led_effects[i].blink_effect.timer);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Failed to delete blink effect timer: %s", esp_err_to_name(err));
            return err;
        }
 ...
 *
 * This function stops the active breath effect for the specified LED. It stops and deletes the associated timer,
 * turns off the LED, resets the breath effect data, and updates the effect state to indicate that no effect is active.
 *
 * @param led Pointer to the LED structure.
 * @return
...
esp_err_t rgb_led_stop_breath_effect(rgb_led_t *led)
{
    ...
    if (led_effects[i].effect_state == RGB_LED_EFFECT_BREATH) {
        ...
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Failed to stop breath effect timer: %s", esp_err_to_name(err));
            return err;
        }

        err = esp_timer_delete(led_effects[i].breath_effect.timer);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Failed to delete breath effect timer: %s", esp_err_to_name(err));
            return err;
        }
...
/**
 * @brief Callback function for transitioning the LED color back to the previous color.
 *
 * This callback function is called when the transition timer expires. It transitions the LED color
 * back to the previous color that was stored before the transition started. The function is intended
 * to be used internally within the RGB LED library and should not be called directly.
 *
 ...
 *
 * This function is called periodically to update the LED brightness for the breath effect.
 *
 * @param arg Pointer to the argument (breath_leds struct) containing breath effect data.
 */
 ...
 *
 * This function is called periodically to update the LED state for the blink effect.
 *
 * @param arg Pointer to the argument (blink data) containing the blink effect parameters.
 */
 ...
 * @return esp_err_t ESP_OK if successful, or an error code if the breath effect cannot be started.
 */
esp_err_t rgb_led_start_breath_effect(rgb_led_t *rgb_led, uint32_t color, uint16_t duration_ms)
{
    if (!rgb_led) {
        ESP_LOGE(TAG, "Invalid LED pointer");
        return ESP_ERR_INVALID_ARG;
    ...
    if (led_effects[i].effect_state == RGB_LED_EFFECT_NONE) {
        ...
        esp_timer_create_args_t timer_args = {
            .callback = breath_effect_callback,
        ...
        led_effects[i].breath_effect.step = 0.05; // Step size for increasing/decreasing brightness
 ...
 *
 * This function starts a blink effect for the specified LED with the given parameters.
 * The LED will alternate between the specified color and off state for the specified number of pulses.
 * The `time_on` parameter specifies the duration in milliseconds for which the LED will be on,
 * while the `time_off` parameter specifies the duration in milliseconds for which the LED will be off.
 * The `time_out` parameter specifies an optional timeout period in milliseconds after which the blink effect will stop automatically.
 *
 * @param led Pointer to the LED structure.
 * @param color The color to blink (RGB value).
 * @param pulse_count The number of pulses for the blink effect.
 * @param time_on The duration in milliseconds for which the LED will be on during each pulse.
 * @param time_off The duration in milliseconds for which the LED will be off during each pulse.
 * @param time_out The optional timeout period in milliseconds after which the blink effect will stop automatically. Set to 0 to disable timeout.
 * @return
 *     - ESP_OK if the blink effect was successfully started.
 *     - ESP_ERR_INVALID_ARG if the input argument is invalid.
 *     - ESP_FAIL if the LED is not found in the LED effects array or the maximum number of LED effects is reached.
 *     - Other error codes if there was an error starting the timer.
 */
esp_err_t rgb_led_start_blink_effect(rgb_led_t *rgb_led, uint32_t color, uint8_t pulse_count, uint32_t time_on, uint32_t time_off, uint32_t time_out)
{
    if (!rgb_led) {
        ESP_LOGE(TAG, "Invalid LED pointer");
        return ESP_ERR_INVALID_ARG;
    ...
    case RGB_LED_EFFECT_BLINK:
        ...
        ESP_LOGI(TAG, "Blink effect updated for LED");
        return ESP_OK;
 ...
 *
 * This function deinitializes the RGB LED module for the specified LED. It stops and deletes any active effects
 * associated with the LED and removes it from the module's internal data structure.
 *
 * @param rgb_led_cfg Pointer to the RGB LED configuration structure.
 * @return
 *     - ESP_OK if the LED was successfully deinitialized.
 *     - ESP_ERR_INVALID_ARG if the input argument is invalid.
 *     - ESP_FAIL if the LED is not found in the module's internal data structure.
 */
